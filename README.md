...
* **[Issue] faster-whisper가 업로드된 음성/영상 파일을 읽는 단계에서 죽은 오류**
  * 업로드된 MP4 파일에 오디오 트랙이 존재하지 않아 faster-whisper가 음성 데이터를 정상적으로 디코딩하지 못하는 문제 발생
  * PyAV를 통해 업로드 파일의 미디어 스트림을 확인한 결과, 해당 파일에는 video (h264) 스트림만 존재하고 audio 스트림이 없는 것으로 확인
  * 오디오 스트림이 없는 파일이 Whisper 처리 단계로 전달되면서 IndexError: tuple index out of range 오류가 발생하고 해당 작업(Job)이 실패하는 이슈 발생

- **[Fix] faster-whisper 업로드 파일 처리 단계에서 발생하는 오류 수정**
  - 업로드된 영상 파일에 오디오 트랙이 존재하지 않는 경우 `faster-whisper`가 음성 데이터를 정상적으로 디코딩하지 못하는 문제 발생
  - PyAV를 활용하여 업로드된 미디어 파일의 스트림을 사전에 확인하고, **오디오 스트림 존재 여부를 검증하는 로직 추가**
  - 오디오 트랙이 없는 파일이 Whisper 처리 단계로 전달되지 않도록 사전 검증하여 `IndexError: tuple index out of range` 오류 방지
  - 오디오 스트림이 없는 파일 업로드 시 사용자가 원인을 확인할 수 있도록 **명확한 오류 메시지 제공**
  - `WhisperModel` 설정을 명시적으로 분리하여 `small` 모델, CPU, `int8` 연산 환경에서의 전사 처리 구조 개선
  - 음성 전사 과정에서 발생하는 예외를 처리하여 작업(Job) 실패 원인을 사용자에게 보다 명확하게 전달하도록 개선
