...
- **[Issue] faster-whisper가 업로드된 음성/영상 파일을 처리하는 과정에서 오류 발생**
  - 업로드된 MP4 파일을 확인한 결과, **video (H.264) 스트림만 존재하고 audio 스트림이 없는 것**으로 확인
  - 오디오 트랙이 없는 파일이 `faster-whisper`의 음성 디코딩 단계로 전달되면서 `IndexError: tuple index out of range` 오류 발생
  - 이로 인해 해당 전사 작업(Job)이 정상적으로 완료되지 않고 실패하는 문제 발생

- **[Fix] 업로드 파일의 오디오 스트림 사전 검증 및 전사 처리 구조 개선**
  - PyAV를 활용하여 업로드된 미디어 파일의 스트림 정보를 사전에 확인하도록 수정
  - **audio 스트림의 존재 여부를 검증하는 로직 추가**
  - 오디오 트랙이 없는 파일은 `faster-whisper` 처리 단계로 전달되지 않도록 사전 차단
  - 오디오 스트림이 없는 파일 업로드 시 사용자가 문제의 원인을 확인할 수 있도록 **명확한 오류 메시지 제공**
  - `IndexError: tuple index out of range`와 같은 내부 예외가 발생하지 않도록 업로드 파일 검증 및 예외 처리 개선
  - `WhisperModel` 설정을 별도로 관리하여 모델 크기, 연산 장치 및 계산 타입을 명확하게 구성
  - 기존 `small` 모델에서 **`large-v3-turbo` 모델로 변경하여 한국어 음성 전사 정확도 개선을 시도**
  - CPU 환경에서 `int8` 계산 타입을 사용하도록 구성하여 로컬 환경에서의 전사 처리 가능하도록 유지
  - 음성 전사 과정에서 발생하는 예외를 처리하고, 작업 실패 원인을 로그와 사용자 화면에서 보다 명확하게 확인할 수 있도록 개선

[다음과 같은 문제 상황이 발생]
<img width="1268" height="831" alt="스크린샷 2026-09-25 154929" src="https://github.com/user-attachments/assets/9d932b94-86ef-45ed-a437-5a14480f407e" />
